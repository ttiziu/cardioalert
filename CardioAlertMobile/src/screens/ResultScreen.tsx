...
const RECOMMENDATION = {
  normal:
    'No se detectan patrones anómalos en la ventana analizada. Continúe el monitoreo.',
  ...
  return (
    <Screen>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <FadeIn>
          <Pulse active={anomaly} periodMs={1800} maxScale={1.01}>
            <Card
              borderColor={color}
              style={[
                styles.hero,
                { backgroundColor: `${color}14`, shadowColor: color },
              ]}
            >
        ...
        <FadeIn delay={160}>
          ...
          <Card style={styles.gap}>
            {LABELS.map(l => (
              <View key={l} style={styles.probRow}>
                <View style={styles.probHead}>
                  <Text
                    style={[
                      styles.probLabel,
                      l === p.label && { color: labelColor[l] },
                    ]}
                  >
                    {labelName[l]}
                  </Text>
                  <Text
                    style={[
                      styles.probValue,
                      { color: l === p.label ? labelColor[l] : colors.muted },
                    ]}
                  >
                    {(p.probabilities[l] * 100).toFixed(1)}%
                  </Text>
                </View>
                <ConfidenceBar
                  value={p.probabilities[l]}
                  color={l === p.label ? labelColor[l] : colors.muted}
                />
              </View>
            ))}
        ...
        <FadeIn delay={240}>
          ...
          <Card>
            <Row
              ...
              value={app.hr !== null ? `${app.hr} BPM` : '--'}
            />
            <Row
              label="Intervalo R-R"
              value={app.rrMs !== null ? `${app.rrMs} ms` : '--'}
            />
        ...
        <FadeIn delay={320}>
          <Card style={styles.recommendation}>
            ...
            <Text style={styles.disclaimer}>
              Herramienta de soporte a la decisión: no reemplaza el criterio del
              profesional.
            </Text>
          </Card>
        </FadeIn>
...
const styles = StyleSheet.create({
  ...
  probLabel: { color: colors.muted, fontSize: 13 },
  probValue: { fontSize: 13, fontWeight: '700', fontFamily: mono },
